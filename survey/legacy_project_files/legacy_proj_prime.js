...
function checkWordCount(){
    s = document.getElementById("legacy_essay").value;
    s = s.replace(/(^\s*)|(\s*$)/gi,"");
    s = s.replace(/[ ]{2,}/gi," ");
    s = s.replace(/\n /,"\n");
    if (s.split(' ').length <= 20) {
			$("#more_words_alert").html('<font color="red">' +
					 'Please type an answer to the question. Your answer should be approximately a half page.' +
					 '</font>');
return false;
    }
		else experiment.next();
}
