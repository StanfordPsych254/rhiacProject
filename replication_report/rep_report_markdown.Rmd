...
The original study reports cohen's d's ranging from 0.23 to 0.36 on the various measures used (legacy motives, climate change beliefs, behavioral intentions, and donation measure). Therefore, I've run power analysis on one of the main effects, the effect of condition on behavioral intentions (d=0.36)

```{r power95}
power.t.test(delta=0.36, power=.95, sig.level=0.05)
```
```{r power90}
power.t.test(delta=0.36, power=.90, sig.level=0.05)
```
```{r power80}
...
###Planned Sample
The original paper uses a sample size of 312 participants, based on the estimated effect size from a pilot test. This is the most feasible number of participants that will not severely underpower the study. I will be adding an additional 5% for a total of 328 participants, to account for any possible exclusions.

```{r}
power.t.test(delta=0.36, sig.level=0.05, n = 164)
```

Using a sample of 312 participants, the smallest sampl

```{r}
power.t.test(power=.8, sig.level=0.05, n = 164)
```

Given a sample of 312 participants, the project has 80% power to detect an effect size greater than d=0.31. 

###Materials & Procedure

**Writing activity**
Participants will be assigned to one of two conditions. As per the original experiment, "In the legacy condition, participants began by writing a short essay describing what they want to be remembered for by future generations...The essay-writing task was omitted in the control condition, which otherwise followed the same procedure as the legacy condition." The writing task will be copied verbatim from the project's supplemental materials online.
...
In the original study, all participants were paid the same amount. However, in this study, we will pay a base rate of $0.20, and an additional bonus of $0.40 to the people in the legacy motives priming condition (ie. the people who write the 4 minute essay). 

In addition, in the original study the average time taken to write the essay was 6.5 minutes. However, we will be shortening the essay writing task to 4 minutes instead. In addition, a 20 word minumum was imposed on the essay task before submission was allowed. There was no countdown indicator, but participants were asked to write more if they tried to submit an essay with under 20 words.
...
id <- data.frame(workerid = jd$WorkerId, 
                ...
                totalHiddenTime = df$totalHiddenTime,
